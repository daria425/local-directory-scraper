...
def get_regional_categories(region:str):
    ...
    if region=="camden":
        categories=CamdenContentReader(category_page).get_main_categories(as_json=True)
    elif region=="islington":
        categories=IslingtonContentReader(category_page).get_main_categories(as_json=True)
    return categories
